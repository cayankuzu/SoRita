...
export function validateManifestSchema(manifest) {
  if (!validateSchema(manifest)) {
    fail(
      `Evidence manifest violates JSON Schema: ${formatSchemaErrors(validateSchema.errors)}`,
    );
  }
  return manifest;
}
...
export function parseArguments(argv) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (option === "--manifest") parsed.manifest = value;
      ...
      parsed.results.push(parseNamedValue(value, option));
...
function repositoryMetadata(cwd) {
  ...
  if (!SHA_PATTERN.test(commitSha))
    fail("Git HEAD is not a full immutable commit SHA");
...
function normalizeArtifact(cwd, artifactPath) {
  ...
  if (workspacePath.startsWith("../") || workspacePath === "..") {
    fail(
      `Evidence artifact must be inside the repository workspace: ${artifactPath}`,
    );
  }
  return {
    path: workspacePath,
...
function normalizeResults(results) {
  ...
  for (const [name, state] of results) {
    if (!/^[a-z0-9][a-z0-9._-]*$/u.test(name))
      fail(`Invalid result name: ${name}`);
    if (!CHECK_STATES.has(state))
      fail(`Invalid result state for ${name}: ${state}`);
    if (Object.hasOwn(normalized, name)) fail(`Duplicate result name: ${name}`);
    normalized[name] = state;
  }

  const missingChecks = REQUIRED_RELEASE_CHECKS.filter(
    (name) => !Object.hasOwn(normalized, name),
  );
  if (missingChecks.length > 0) {
    fail(`Missing required release checks: ${missingChecks.join(", ")}`);
  }

  const unknownChecks = Object.keys(normalized).filter(
    (name) => !REQUIRED_RELEASE_CHECK_SET.has(name),
  );
  if (unknownChecks.length > 0) {
    fail(
      `Unknown release checks for schema v${SCHEMA_VERSION}: ${unknownChecks.join(", ")}`,
    );
  }
...
function resolveManifestArtifact(cwd, artifact) {
  ...
  if (
    ...
    fail(
      `Evidence artifact must use a normalized repository-relative path: ${artifact.path}`,
    );
  }
...
export function verifyManifest({ cwd, manifestPath }) {
  ...
  if (manifest.commitSha !== currentSha) {
    fail(
      `Manifest SHA ${manifest.commitSha} does not match HEAD ${currentSha}`,
    );
  }
  ...
  for (const artifact of manifest.artifacts) {
    ...
    if (
      !existsSync(absoluteArtifactPath) ||
      !statSync(absoluteArtifactPath).isFile()
    ) {
