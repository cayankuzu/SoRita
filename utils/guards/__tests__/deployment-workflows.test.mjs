...
test('preview Worker deployment is protected, strict and deletes transient secrets', () => {
  ...
  assert.match(cloudflarePreview, /rm -f -- .*sorita-edge-preview-secrets\.json/u);
...
test('production Worker deploy requires same-SHA gates and only starts a 5% canary', () => {
  ...
  assert.match(cloudflareProduction, /actions\/runs\/\$\{PREVIEW_RUN_ID\}\/jobs/u);
...
test('production deploys consume one exact same-run release evidence artifact before mutation', () => {
  for (const [name, source, mutationMarker] of [
    ['Cloudflare', cloudflareProduction, 'Upload a production version without routing traffic'],
    ['EAS', easProduction, 'Publish an initial 5% production rollout'],
  ]) {
    assert.match(source, /release_evidence_run_id:/u, `${name} must require an evidence run ID`);
    ...
    assert.match(source, /artifact_name="partial-release-evidence-\$\{TARGET_SHA\}"/u);
    ...
    assert.match(source, /gh run download "\$RELEASE_EVIDENCE_RUN_ID"/u);
    assert.match(source, /--repo "\$GITHUB_REPOSITORY"/u);
    assert.match(source, /--name "\$artifact_name"/u);
    assert.match(source, /git status --porcelain=v1 --untracked-files=all/u);
    assert.match(source, /npm run release-evidence:verify -- --manifest "\$RELEASE_MANIFEST"/u);
    ...
    assert.match(source, /manifest\.release\?\.runtimeVersion !== process\.env\.EXPECTED_RUNTIME_VERSION/u);
    const cleanCheckoutIndex = source.indexOf('git status --porcelain=v1 --untracked-files=all');
    const downloadIndex = source.indexOf('gh run download "$RELEASE_EVIDENCE_RUN_ID"');
    const verifierIndex = source.indexOf('npm run release-evidence:verify');
    assert.ok(cleanCheckoutIndex < downloadIndex, `${name} must prove a clean checkout before download`);
    assert.ok(downloadIndex < verifierIndex, `${name} must download the selected artifact before verifying it`);
    assert.ok(
...
test('release evidence remains checksum-bound and explicitly incomplete without runtime evidence', () => {
  assert.match(releaseEvidence, /candidate_sha/u);
  assert.match(releaseEvidence, /WORKFLOW_SHA: \$\{\{ github\.sha \}\}/u);
  assert.ok(releaseEvidence.includes('${CANDIDATE_SHA,,}" != "${WORKFLOW_SHA,,}'));
  assert.match(releaseEvidence, /runtime_version must match the immutable candidate package version/u);
  ...
  assert.match(releaseEvidence, /incomplete evidence manifest was accepted unexpectedly/iu);
  assert.match(releaseEvidence, /name: partial-release-evidence-\$\{\{ inputs\.candidate_sha \}\}/u);
});
...
test('deployment workflows avoid privileged PR triggers and mobile service-role material', () => {
  for (const source of deploymentWorkflows) {
    ...
    for (const match of source.matchAll(/uses:\s+(actions\/(?:checkout|setup-node|upload-artifact)|expo\/expo-github-action)@([^\s]+)/gu)) {
      assert.match(match[2], /^[0-9a-f]{40}$/u, `${match[1]} must be SHA-pinned`);
    }
  }
});

test('production and release-evidence workflows do not introduce Docker', () => {
  for (const source of [cloudflareProduction, easProduction, releaseEvidence]) {
    assert.doesNotMatch(source, /\bdocker\b/iu);
  }
});
