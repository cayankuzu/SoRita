...
import { persistLocalUriToFile } from '@/mobile/app/platform/media/localFiles';
import {
  buildPickedMediaPath,
  generatePlacePhotoThumbnailUri,
  optimizePickedImageAsset,
} from '@/mobile/app/platform/media/pickedMediaFiles';
import type {
  CameraCaptureMode,
...
// A single image with a crop goes through the native editor.
function shouldUseNativeEditing(
  options: Pick<PickImagesOptions, 'allowMultiple' | 'cropAspect' | 'cropShape'> = {},
) {
...
async function pickImagesFromPrompt(options: MediaPickerPromptOptions = {}) {
  ...
  if (selection.source === 'library') {
    if (shouldUseNativeEditing(options)) {
