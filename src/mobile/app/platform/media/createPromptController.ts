...
/**
 * Generic factory for creating promise-based UI prompt controllers.
 * Used for media pickers, camera capture, library selection, and similar
 * flows where we present a modal and await the user's response.
 */
export function createPromptController<TOptions extends object, TResult>(
  ...
  function open(options: TOptions = defaultOptions): Promise<TResult | null> {
    ...
    emitChange();
    return new Promise<TResult | null>((resolve) => { activeResolver = resolve; });
  }
