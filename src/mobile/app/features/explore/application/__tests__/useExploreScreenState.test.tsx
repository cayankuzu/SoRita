...
describe('useExploreScreenState', () => {
  ...
  it('filters discoverable content and follows users through the mutation layer', async () => {
    ...
    expect(hook.result.current.filteredUsers.map((item) => item.id)).toEqual(['followed', 'public-user']);
    expect(hook.result.current.filteredListItems.map((item) => item.list.id)).toEqual(['followed-list', 'public-list']);
    expect(hook.result.current.filteredPlaces.map((item) => item.place.id)).toEqual(['place-2', 'place-1']);
    expect(hook.result.current.filteredPhotos.map((item) => item.place.id)).toEqual(['place-2', 'place-1']);
  ...
  it('handles empty sessions and blocks follow actions without an active user', async () => {
    ...
    await expect(hook.result.current.followUser('public-user')).rejects.toThrow(
      'Takip islemi icin aktif kullanici gerekli.',
    );
  });
});
