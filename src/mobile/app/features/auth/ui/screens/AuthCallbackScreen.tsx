...
export function AuthCallbackScreen() {
  ...
  useEffect(() => {
    ...
    void completeSignup().catch((error) => {
      fail(error instanceof Error ? error.message : 'Dogrulama tamamlanamadi.');
    });
    ...
    return (
      <Screen scroll={false}>
        <View style={styles.centered}>
          ...
          <Text style={styles.title}>Hesabin dogrulaniyor</Text>
          <Text style={styles.description}>SoRita birazdan acilacak.</Text>
        </View>
      </Screen>
    );
  }
    ...
    <Screen scroll={false}>
      <View style={styles.centered}>
        <View style={styles.errorCard}>
          <Text style={styles.title}>Baglanti dogrulanamadi</Text>
          <Text style={styles.description}>{screenState.message}</Text>
          <PrimaryButton
            title="Giris ekranina don"
