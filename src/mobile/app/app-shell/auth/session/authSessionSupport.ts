...
function setVisibleDataContextCache(userId: string, context: VisibleDataContext) {
  const previousSnapshot = queryClient.getQueryData<VisibleDataSnapshot>(
    queryKeys.visibleData.snapshot(userId),
  );

  queryClient.setQueryData(queryKeys.visibleData.context(userId), context);
  queryClient.setQueryData(queryKeys.visibleData.snapshot(userId), {
    ...context,
    lists: previousSnapshot?.lists ?? [],
  });
}
...
export async function restorePersistedVisibleDataSnapshot(authUserId: string) {
  ...
  queryClient.setQueryData(queryKeys.visibleData.context(authUserId), {
  ...
  queryClient.setQueryData(
    queryKeys.visibleData.lists(authUserId, { pageSize: 20 }),
    createInfiniteQueryCachePage(snapshot.lists.slice(0, 20)),
  );
  queryClient.setQueryData(queryKeys.visibleData.snapshot(authUserId), snapshot);
  return snapshot;
}
...
function isEmailConfirmed(authUser: SupabaseAuthUser) {
  return Boolean(
    (authUser as SupabaseAuthUser & { email_confirmed_at?: string | null }).email_confirmed_at,
  );
}
...
export async function ensureProfileExists(authUser: SupabaseAuthUser) {
  ...
  const { error: insertError } = await supabase.from('profiles').insert({
    ...
    name: String(metadata.name ?? metadata.full_name ?? authUser.email?.split('@')[0] ?? tr.auth.defaultUserName),
...
export async function resolveCurrentUser(authUserId: string | null): Promise<User | null> {
  ...
  setVisibleDataContextCache(authUserId, context);
...
export async function hydratePersistedAuthState(authUserId: string) {
  ...
  void refreshNotifications(authUserId)
    ...
    .catch((err) => { logger.debug('auth', 'Failed to refresh notifications after hydrating persisted auth state', err); });
...
export async function syncAuthenticatedUser(authUser: SupabaseAuthUser): Promise<User | null> {
  ...
  void refreshNotifications(authUser.id)
    ...
    .catch((err) => { logger.debug('auth', 'Failed to refresh notifications after syncing authenticated user', err); });
