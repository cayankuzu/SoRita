...
export async function updateUser(user: User) {
  assertNoObjectionableContent([
    { label: 'Ad alani', value: user.name },
    { label: 'Kullanici adi', value: user.username },
    { label: 'Biyografi', value: user.bio },
  ]);
  ...
  return (
    (await fetchUserByIdIncludingBlocked(user.id)) || {
      ...user,
...
export async function reportUser(reporterUserId: string, targetUserId: string, reason: string) {
