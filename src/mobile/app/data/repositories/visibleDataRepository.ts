...
function buildPostgrestInList(values: string[]) {
...
async function fetchUsersAndBlocks(
  pageLimits: VisibleDataPageLimits,
  ...
  const profileIds = Array.from(
    new Set([
      ...((profiles || []) as PublicProfileRow[]).map((profile) => profile.id),
      ...(viewerId ? [viewerId] : []),
    ]),
  );
  const profileIdFilter = buildPostgrestInList(profileIds);
  const [
    ...
    profileIds.length
      ? supabase
          ...
          .or(`follower_id.in.${profileIdFilter},following_id.in.${profileIdFilter}`)
      : Promise.resolve({ data: [], error: null }),
    profileIds.length
      ? supabase
          .from('follow_requests')
          .select('id, requester_id, target_user_id, status, created_at, responded_at')
          .or(`requester_id.in.${profileIdFilter},target_user_id.in.${profileIdFilter}`)
      : Promise.resolve({ data: [], error: null }),
    viewerId
      ? supabase
          .from('user_blocks')
...
export async function fetchVisibleDataContext(userId?: string | null): Promise<VisibleDataContext> {
  ...
  const { users, blockRows } = await fetchUsersAndBlocks(pageLimits, viewerId);
...
export async function fetchUserByIdIncludingBlocked(userId: string) {
  const { users } = await fetchUsersAndBlocks(DEFAULT_VISIBLE_DATA_LIMITS, userId);
