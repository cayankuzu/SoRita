...
describe('visibleDataRepository', () => {
  ...
  it('supports public snapshots without a viewer id', async () => {
    ...
    expect(listsQuery?.eq).toHaveBeenCalledWith('is_public', true);
  });
  ...
  it('builds a context-only view and paginated list queries with filters', async () => {
    ...
    expect(context.currentUser?.id).toBe('viewer');
    expect(ownerListsQuery?.eq).toHaveBeenCalledWith('owner_id', 'viewer');
    expect(publicListsQuery?.eq).toHaveBeenCalledWith('is_public', true);
    expect(listByIdQuery?.eq).toHaveBeenCalledWith('id', 'list-1');
