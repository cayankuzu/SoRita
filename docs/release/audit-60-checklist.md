...
| 9B: Premium görsel tur | 🔶 | Cayan uygulamayı telefonda büyük buldu: bütün boyut token'ları bir basamak küçüldü (yazı 12–28 → 11–26, ikon, avatar, buton 48 → 40, üst bar 56 → 48, sekme çubuğu 60 → 56), dokunma hedefleri 48 dp (OTA 41, `3836b1f`). 2. tur telefondaki görüntülerden: kontroller 40 dp boyanıyor, profil kapağı 144 / avatar 80, harita arama ve butonları 40, ayar satırları 48, harita filtre satırları 36 (OTA 42 `b32bfb6`, OTA 43 `beed7ac`). 4. tur: yer ve liste düzenleyicisi, bildirim satırları, medya seçici 40 dp (OTA 44 `258b0c0`). 48 dp boyanan kutu 69 → 7; kalanlar bilerek: sekme çubuğu, eylem menüsü satırları, giriş ekranları. Dört sekme, ayarlar, liste detayı, bildirimler ve yer düzenleyicisi cihazda kontrol edildi |
| 9A: Durum matrisi, motion, cila | 🔶 | A6 ilk tur: "siz" diliyle kalan 5 metin "sen"e çekildi, "Bir hata oluştu" yok. B14: hareketi azalt ayarı artık 8 animasyonun 8'inde (yakınlaştırma ve üst bar eklendi). OTA 45 `8e6dce1`. Kalan: A4 uçak modu / sunucu hatası matrisi (sistem ayarı, Cayan onayı), B16 açılış geçişi, native izin metinlerindeki "siz" dili (native paket #2) |
...
| F37 çevrimdışı | 🔶 | **P0 bulundu ve düzeltildi.** Cayan uçak modunu açıp uygulamayı açınca hesabından çıkış yapıldı. Sebep: istemci oturumu kendisi saklamıyor (`persistSession: false`), açılışta `restorePersistedSession` kayıtlı kopyayı `setSession`e veriyor, o da süresi dolmamış anahtarda bile Supabase'e gidiyor; çevrimdışı hata alınca tek kopya siliniyordu. Artık yalnız Supabase'in açık reddi (408/429 dışındaki 4xx) oturumu bitiriyor; ağ hatası, zaman aşımı ve 5xx `SessionRestoreDeferredError` olarak işleniyor, açılış kayıtlı kullanıcı ve önbellekle sürüyor, oturum dakikada bir ve bağlantı gelince yeniden doğrulanıyor (`ba5b2ee`, OTA 46, cihazda yüklü). Cihazda doğrulandı: Cayan yeniden girdi, uçak modunda soğuk açılışta log "Auth bootstrap could not reach Supabase; keeping the session" dedi ve önbellekteki akış açıldı; internet gelince soğuk açılış oturumu açık buldu. Aynı loglarda ikinci hata: çevrimdışıyken "Push registration or cleanup failed" saniyede 10–20 kez; kayıt artık çevrimdışıyken denenmiyor, bağlantı dönünce hemen yapılıyor (`be7dd36`, OTA 47, cihazda yüklü; çevrimdışı tekrar testi bekliyor). 12 ekranın tablosu için uçak modunu Cayan açıp kapatıyor |
