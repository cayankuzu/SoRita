...
| CF-16 | Origin `Set-Cookie`, cache header, HTML/redirect, aşırı büyük/geçersiz JSON veya hassas hata body enjekte eder | Worker response header'larını yeniden kurar; cookie/cache aktarmaz; body'leri byte-limitli okur; 200 cevapları route/action şemasıyla, 4xx cevapları dar error envelope ile doğrulayıp yeniden serialize eder; redirect/5xx/204/şema dışı cevapları sanitize eder | Sözleşme değişiklikleri Worker ve origin testlerinde birlikte güncellenmelidir | MEDIUM |
...
| CF-18 | Yanlış production URL/CORS/namespace/timeout veya açık `workers.dev` ile config drift | Runtime config fail-closed; üç named env; production `workers_dev: false`, `preview_urls: false` ve route yokken erişilebilir ingress yok; placeholder değerler açıkça işaretli; generated types/test | Dashboard ile checked-in config karşılaştırması, unique namespace ve onaylı production custom route gerekir | **HIGH — UNVERIFIED, NO-GO** |
