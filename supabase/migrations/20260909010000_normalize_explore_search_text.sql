-- Keep Explore search consistent with the client for Turkish letters while
-- preserving the existing RPC signature, visibility rules, and pagination.
...
as $$
  ...
  list_items as (
    ...
    where auth.uid() is not null
      ...
      and (
        normalized.q = ''
        or pg_catalog.strpos(private.normalize_search_text(lists.name), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(lists.description), normalized.q) > 0
      )
  ),
  place_items as (
    select
    ...
    where auth.uid() is not null
      ...
      and (
        normalized.q = ''
        or pg_catalog.strpos(private.normalize_search_text(list_places.name), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(list_places.address), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(list_places.notes), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(lists.name), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(owner_profile.name), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(owner_profile.username), normalized.q) > 0
      )
  ),
  user_items as (
    select
    ...
    where auth.uid() is not null
      ...
      and (
        normalized.q = ''
        or pg_catalog.strpos(private.normalize_search_text(profiles.name), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(profiles.username), normalized.q) > 0
        or pg_catalog.strpos(private.normalize_search_text(profiles.bio), normalized.q) > 0
      )
  ),
  combined as (
